...
一：对于行内元素采用`text-align:center;`的方式



二：采用`margin:0 auto;`来实现水平居中显示
...
五：父子元素都采用相对定位，父元素`left:50%;`子元素`left:-50%;`相对自己的长度减回50%，这样实现向右偏移后拉回多的部分



六:采用`css3`的`flexbox，display:flex;`



七：用父元素的 `display:relative;`直接采用`position:absolute;left:0;right:0;margin:auto`来实现水平以居中
